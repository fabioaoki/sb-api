...
public class AccountStatusService {
	...
	public List<AccountStatusDto> findAccountStatus() {
		...
		for (AccountStatus accountStatusList : accountStatus) {
			...
			SituationAccount accountSituation = Enum.valueOf(SituationAccount.class,
					accountStatusList.getAccountSituation().toString());
			accountStatusDtos.add(AccountStatusDto.builder().id(accountStatusList.getId())
					.dateModify(accountStatusList.getDateModify()).accountSituation(accountSituation)
					.account(AccountDto.builder().id(account.getId()).build()).build());
		}
		return accountStatusDtos;
	}

	public List<AccountStatusDto> findAccountStatusById(long id) throws AccountStatusException, AccountException {
		Account account = accountRepository.findById(id);
			...
			if(Objects.nonNull(accountStatus)) {
				...
				for (AccountStatus accountStatusList : accountStatus) {
					SituationAccount accountSituation = Enum.valueOf(SituationAccount.class,
							accountStatusList.getAccountSituation().toString());
					accountStatusDtos
							.add(AccountStatusDto.builder().id(accountStatusList.getId())
									.dateModify(accountStatusList.getDateModify()).accountSituation(accountSituation)
									.account(AccountDto.builder().id(account.getId()).number(account.getNumber())
											.amount(account.getAmount()).dateCreate(account.getDateCreate()).build())
									.build());
				}
				return accountStatusDtos;

			} else {
				throw new AccountException("Historico nao encontrado");
			}
			
		} else {
			throw new AccountException("Id de conta nao encontrada");
		} 
	}
}
